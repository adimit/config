...
(defun word-boundary-p (prev next)
  ...
  (pcase `(,prev ,next)
    ...
    ;;(`(newline whitespace) t)
    (`(punctuation newline) t)
    ;;(`(newline punctuation) t)
    (_ nil)))
...
#+begin_src emacs-lisp
(defun go-to-or-beginning-of-buffer (&optional n)
  "Go to line N or to the beginning of the buffer."
  (interactive "^P")
  (message "prefix %s" (or current-prefix-arg n))
  (if (or current-prefix-arg n)
      (progn
        (goto-char (point-min))
        (forward-line (1- (prefix-numeric-value (or current-prefix-arg n)))))
    (goto-char (point-min))))

(transient-define-prefix dispatch-goto-menu () "This isn't documentation"
  [["Move"
    ("e" "bottom" end-of-buffer)
    ("g" "top" go-to-or-beginning-of-buffer)
