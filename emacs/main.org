...
Add =graphviz-dot-mode= and its =company= extension to facilitate writing of graphviz graphs
...
Support for =systemd= unit files. Allows browsing help for systemd using =C-c C-o=. It should automatically get loaded for stuff in =/etc/systemd/= and files ending in =.service=. Includes a company backend.

It does have a few bugs. The [[https://github.com/holomorph/systemd-mode/issues/16][company-backend]] isn't working properly (so we attempt to work around that) and flycheck integration also doesn't seem to work.
