...
*** Work around null bytes in JSON response

=ts-ls= sends null bytes sometimes in responses like these:

#+begin_src json
{"jsonrpc":"2.0","id":103,"result":{"items":[{"label":"extraOptions?","kind":5,"sortText":"12\u0000extraOptions\u0000"}]}}
#+end_src

(This snippet is abbreviated.)

That makes JSON processing inside Emacs quite upset, and it errors out with:

#+begin_src
  json-parse-error \u0000 is not allowed without JSON_ALLOW_NUL
#+end_src

We just replace all null bytes before they're sent to =json-parse= here to avoid bugs in LSP-servers causing problems.


The suggestion comes from a response to a  [[https://github.com/emacs-lsp/lsp-mode/issues/2681][github bug on LSP]] for a similar issue. The clean way to fix this is to avoid null bytes in =typescript-language-server=. However, [[https://github.com/typescript-language-server/typescript-language-server/issues/559][they don't think it's their problem]], but that it's coming from TS-side. In any case, Emacs 29 is probably going to make that problem go away, too.

Note that LSP actually uses =json-parse-buffer= in its =lsp--create-filter-function= (via its own macro =lsp-json-read-buffer=). Since =json-parse-buffer= is C-code, it doesn't actually use =json-parse-string= which was patched above. We need to instead patch =json-parse-buffer=.
...
This solution may not be terribly fast. In particular, I'm not at all sure that the =(while (re-search-forward needle …))= loop is the most optimal way to replace all occurrences of =needle= in the current buffer. But it gets the job done.
