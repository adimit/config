...
*** DONE Redo
*** DONE Start meow global mode on startup
Something's preventing it from just coming online. I have to call =meow-setup= and =meow-global-mode= by hand
...
*** TODO Find a way to join lines easily
Vim's =shift-k=. How does that work in Helix? I seem to recall that it didn't work there either.
*** DONE Fix =o= and =O=
They should work like in Helix and Vim.
=A= does sort of what I want, I think.
*** DONE Add =A= keybinding
cancel selection, then move to EOL, switch to insert.
*** DONE Remove =general=
That one's only useful with evil mode
*** DONE Add search keybindings
Maybe just use isearch, and but bind it to =/= and =?=
*** TODO Add surround replace
*** TODO Add surround delete
*** DONE Add surround around object
*** DONE Add surround inside object
...
*** DONE Go to references
In LSP mode
#+begin_src emacs-lisp
           ;"g r" 'lsp-ui-peek-find-references
#+end_src
*** DONE Add keybinds for copilot in insert mode
This was the =general= definer for evil:

*** DONE Have =w= behave like in Helix
Meow uses =meow-mark-word= here, where Helix uses something closer to Vim's =w=. The difference between =e= and =w= in Helix is that =e= will select a word's preceding whitespace and skip over whitespace when jumping (so an =e= selection can only start on whitespace, not end on it) whereas a =w= just inverts whatever =b= did transparently, so it can end on whitespace.
*** DONE Add buffer switch to keypad menu
*** DONE Preserve C-W in insert mode

I don't control a lot in insert mode, but =C-w= is important to my mental well-being. We'll need to tell Meow that we don't want =C-w= to do anything with regions later by setting =meow--kbd-kill-region= to =S-<delete>=.
...
(transient-define-prefix dispatch-window-menu () "This isn't documentation"
  [["Movement"
   ...
   ["Control"
    ...
    ("v" "split window vertically" split-window-vertically)]
   ["Winner"
