...
A little snippet to open a shell in the current buffer's working directory.

#+begin_src emacs-lisp
(defun aleks/open-shell-here ()
  "Spawn a shell in the current buffer's directory."
  (interactive)
  (if buffer-file-name
      (let ((target-directory (file-name-directory buffer-file-name)))
        (start-process (format "terminal-in-%s" target-directory) nil "foot" "-D" target-directory "fish" "-C" "ls"))
    (message "Buffer %s is not associated with a file." (buffer-name))))
#+end_src

We'll put both under the global =C-c= keymap.

#+begin_src emacs-lisp
(keymap-global-set "C-c o t" 'aleks/open-shell-here)
(keymap-global-set "C-c o e" 'eshell-here)
#+end_src
...
A mode to edit =jq= expressions. Also allows editing them in org buffers like so:

#+begin_src org
,#+begin_src jq :in-file a.json
.[]
,#+end_src
#+end_src
...
A small major mode to edit KDL files (Niri's weird idea of a configuration file format. I'd rather it have steel or something).

Note that kdl needs to have been compile using =treesit-install-language-grammar=. Also note the extension of =treesit-language-source-alist= further on.
...
#+begin_src emacs-lisp
  ...
  (setq my-org-main-file (concat (file-name-as-directory org-directory) "main.org"))
  (setq my-org-work-file (concat (file-name-as-directory org-directory) "work.org"))
#+end_src
...
(setq org-capture-templates
      `(("c" "Add to today" entry
        ...
        ("j" "Add MP Jira ticket" entry
         (file+headline my-org-work-file "Tasks")
         ,(join-lines
           '("* TODO %(xjira-org-capture-issue \"KNUTH\") %(xjira-get 'issue) %(xjira-get 'title) :ticket:"
             "SCHEDULED: %t"
             ":PROPERTIES:"
             ...
             ":Assignee: %(xjira-get 'assignee)"
             ":Status: %(xjira-get 'status)"
             ":END:")))
        ("r" "Add MP Jira ticket to review" entry
         (file+headline my-org-work-file "Tasks")
         ,(join-lines
           '("* TODO Review %(xjira-org-capture-issue \"KNUTH\") %(xjira-get 'issue) %(xjira-get 'title) :review:"
             "SCHEDULED: %t"
             ":PROPERTIES:"
             ":REFERENCE: %(eval (xjira-make-org-link (xjira-get 'issue) (xjira-get 'title)))"
             ":Parent: %(eval (xjira-make-org-link (xjira-get 'parent) (xjira-get 'parent-title)))"
        ...
        ("m" "Add meeting" entry
         ...
         ,(join-lines
           '("* MEET %?"
             ...
             ":REFERENCE:"
             ":END:")))
        ("M" "Good Morning" entry
         (file+headline my-org-work-file "Tasks")
         ,(join-lines
           `("* TODO KNUTH-114812 %<%A> Morning Checklist"
             "SCHEDULED: %t"
             ":PROPERTIES:"
             ":END:"
             ""
             "- [ ] check emails, teams"
             ,(concat "- [ ] check [[" confluence-tasks-url "][tasks in Confluence]]")
             ,(concat "- [ ] ensure times are [[" time-tracking-url "][tracked for last work]] day")
             ,(concat "- [ ] " student-board-link)
             "- [ ] [[shell:foot zsh][log in]]"
             "- [ ] check gW and Google calendars"
             "- [ ] (re-)schedule agenda"
             "- [ ] block time for today"
             "- [ ] any open code reviews?"
             "- [ ] designate first task for today"
             "")))
        ("S" "Sprint" entry (file my-org-work-file)
         ,(join-lines
           '("* Sprint %^{Title}"
             ...
             "*** MEET Sprint Review"
             "SCHEDULED: %(aleks/org-active-timestamp-from-gregorian (aleks/next-sprint-review-day) '(14 30))"
             "*** MEET Retro"
...
#+begin_src emacs-lisp
(defun inhibit-apheleia-in-package-json ()
  "Return t if the buffer is visiting a file called package.json"
  (string-equal (file-name-nondirectory (or buffer-file-name "")) "package.json"))
...
(use-package apheleia
  ...
  :custom
  (apheleia-inhibit-functions '(inhibit-apheleia-in-package-json)))
#+end_src
...
I use this to mimic how Vim's jump list  =CTRL-O= and =CTRL-I= works.
...
Delete the currently active file & kill its buffer.

#+begin_src emacs-lisp
(defun delete-this-file (&optional trash)
  "Delete the file currently associated with the active buffer."
  (interactive "P")
...
Allow agent workflows with copilot.
...
(use-package gptel
  ...
  (gptel-make-kagi "kagi" :key (lambda () (auth-source-pass-get 'secret "kagi.com/api")))
  (with-eval-after-load 'gptel-transient
    (transient-replace-suffix 'gptel-menu "RET"
      '("C-<return>" gptel--suffix-send))))
#+end_src

Ask kagi something

#+begin_src emacs-lisp
(gptel-make-preset 'kagi
  :description "Ask Kagi about something"
  :backend "kagi"
  :model 'fastgpt)
#+end_src
...
#+begin_src emacs-lisp
(gptel-make-preset 'sum
  :description "Create a summary of a buffer or URL"
  :backend "kagi"
  :model 'summarize:agnes)
#+end_src
...
A little LSP server to make it easier to write home assistant automations etc. Because I sure don't wanna run that VS Code abomination on my Pi.
...
(defun homeassistant-lsp-if-configuration-file ()
  "Check if the current buffer is 'configuration.yaml'."
  (string-equal (file-name-nondirectory (buffer-file-name)) "configuration.yaml"))

(defun homeassistant-fetch-auth (item)
  "Fetch the Home Assistant token from auth source using the key `homeassistant/token`."
  (auth-source-pass-get 'secret (concat "home-assistant/" item)))

(defvar homeassistant-lsp-cmd `(,(expand-file-name "~/.local/share/pnpm/homeassistant-lsp") "--stdio")
  "Command to start the Home Assistant LSP server.")
(with-eval-after-load 'lsp
  (lsp-register-client
   (make-lsp-client
    :new-connection (lsp-stdio-connection (lambda () homeassistant-lsp-cmd))
    :activation-fn 'homeassistant-lsp-if-configuration-file
    :server-id 'homeassistant
    :multi-root t
    :initialization-options (lambda ()
                              (let ((token (homeassistant-fetch-auth "token"))
                                    (host (homeassistant-fetch-auth "host")))
                                (unless (and token host)
                                  (user-error "Failed to fetch the Home Assistant auth info from auth source"))
                                `(:homeassistant
                                  (:host ,(concat "ws://" host "/api/websocket")
                                         :token ,token)))))))
#+end_src

** Hemacs

My own layer on top of meow to bring editing shortcuts closer to Helix.

#+begin_src emacs-lisp
(use-package hemacs
  :straight `(hemacs :type git :repo ,(expand-file-name "src/hemacs" (getenv "HOME")))
  :after (meow)
  :config
  (hemacs--register-hxword))
#+end_src
...
[[https://github.com/blahgeek/emacs-lsp-booster][LSP Booster]] should make lsp somewhat faster. Maybe. Since we're running Emacs 30, the [[https://github.com/blahgeek/emacs-lsp-booster/issues/35][JSON-to-bytecode parser is probably not worth it]], we'll disable that. Note that the maintenance status of lsp-booster seems dubious right now.

You'll need lsp booster first.
...
(use-package magit
  ...
  (with-eval-after-load 'magit-branch
    (transient-append-suffix 'magit-branch "c"
      '("o" "from current task" aleks/create-branch-from-current-task))))
#+end_src
...
*** DONE Add skip paragraph/defun bindings ={= and =}=
...
*** DONE Stop windows from bouncing around when opening transients
...
This just hides transient for 1 second. That's OK for me.

#+begin_src emacs-lisp
(use-package transient
  :custom (transient-show-popup 1))
#+end_src
...
*** DONE Make =w= behave more like in Helix/Vim
...
#+begin_src emacs-lisp :tangle no
(defun aleks/categorise-char (&optional pos)
  "Categorise char after POS into one of the relevant categories for movement.

The categories include: \\='whitespace \\='punctuation \\='word or \\='newline."
...
(defun word-boundary-p (prev next)
  "Returns t if there's a word boundary between PREV and NEXT.

Trailing whitespace is counted as part of the word."
  (pcase `(,prev ,next)
    (`(word punctuation) t)
    (`(punctuation word) t)
    (`(whitespace newline) t)
    (`(whitespace punctuation) t)
    (`(punctuation whitespace) t)
    (`(punctuation newline) t)
    (_ nil)))

(defun bounds-of-hxword-at-point ()
  "Get the bounds of the hxword at point."
  (let* ((end (hxword-end (point)))
         (start (hxword-start end)))
    (cons start (1+ end))))

(defun hxword-at-point ()
  "Get the string of the hxword at point."
  (pcase-let ((`(,start . ,end) (bounds-of-hxword-at-point)))
    (buffer-substring start end)))

(defun hxword-end (position)
  "Find the forward bound of a hxword starting from POSITION."
  (let* ((pos position)
         (prev (aleks/categorise-char pos))
         (next nil))
    (while (and
            (< pos (buffer-size))
            (progn
              (setq next (aleks/categorise-char (1+ pos)))
              (not (word-boundary-p prev next))))
      (setq pos (1+ pos)
            prev next))
    pos))

(defun hxword-start (position)
  "Find the backward bound of a hxword starting from POSITION."
  (let* ((pos position)
         (prev (aleks/categorise-char pos))
         (next nil))
    (while (and
...
(defun forward-hxword (&optional n)
  "Move forward N words or 1 if N is nil or backwards if N is negative."
  (interactive)
  (let ((n (or n 1))
        (pos (point)))
    (if (> n 0)
        (let ((i 0)
              (pos (1+ pos)))
          (while (< i n)
            (setq pos (hxword-end pos)
                  i (1+ i))
            (goto-char (1+ pos))))
      (let ((i (abs n))
            (pos (1- pos)))
        (while (< 0 i)
          (setq pos (hxword-start pos)
                i (1- i))
          (goto-char pos))))))

(defun register-hxword ()
  "Registers hxword as a thing."
  (put 'hxword 'bounds-of-thing-at-point 'bounds-of-hxword-at-point)
  (put 'hxword 'thing-at-point 'hxword-at-point)
  (put 'hxword 'beginning-op (lambda () (forward-hxword -1)))
  (put 'hxword 'end-op (lambda () (forward-hxword 1))))
#+end_src
...
*** DONE Toggle case function
#+begin_src emacs-lisp
(defun aleks/toggle-case ()
  "Toggle case of the char at point.

Moves point forward as a side-effect. This is in line with Vim's
behaviour, but differs from Helix."
  (interactive)
  (if (char-uppercase-p (char-after (point)))
...
#+begin_src emacs-lisp
(defun aleks/is-region-on-lines-p ()
  "Returns t iff the region covers a whole line or lines.
This means that the region starts on BOL and ends on EOL
(not necessily the same line)."
  (and (region-active-p)
       (save-excursion
...
(defun aleks/delete ()
  ...
  (if (region-active-p)
      (progn
        (when (and (aleks/is-region-on-lines-p)
                   (< (point) (point-max)))
...
(defun aleks/goto-line (&optional count)
        "Jump to COUNT line, or to the beginning of the buffer."
        (interactive "p")
...
(defun meow-setup ()
  ...
  (meow-normal-define-key
   ...
   ;;; Emacs treats C-i and TAB the same. There's ways around that but this is easier
   ;;;'("C-i" . back-button-global-forward)
   '("C-S-o" . back-button-global-forward)
...
(use-package meow
  ...
  (("C-w" . aleks/delete-backward-word)
   ...
   ("<f6>" . lsp-rename))
  :custom
  (meow-word-thing 'hxword)
  (meow-cheatsheet-layout meow-cheatsheet-layout-dvorak)
  (meow-expand-hint-counts '((word . 10) (line . 10) (block . 10) (find . 10) (till . 10)))
  :config
...
Highlight parentheses in different colours according to their nesting level. jq mode has problems with it, though, so we disable it there.

#+begin_src emacs-lisp
(use-package rainbow-delimiters
  :hook ((prog-mode . (lambda ()
                        (unless (eq major-mode 'jq-mode)
...
Add a KDL grammar, because Niri made the questionable choice to use KDL as their configuration language.
...
A little helper function to allow quick retrievals of raw contents from a URL. It's wild Emacs doesn't have that built-in.

#+begin_src emacs-lisp
(defun aleks/url-get-into-buffer ()
  "Retrieve the contents of a URL into a temp buffer and switch to it."
  (interactive)
  (request (read-from-minibuffer "URL: ")
    :success (cl-function
              (lambda (&key data &allow-other-keys)
                (with-current-buffer
                    (get-buffer-create "*http temp*")
                  (erase-buffer)
                  (insert data)
