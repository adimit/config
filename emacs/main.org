...
The agenda is a pain with evil, so we have to work around that a little.

#+begin_src emacs-lisp
  (defun set-up-org-agenda-key-bindings ()
    "Hook to set up agenda keybindings"
    (require 'evil-org-agenda)
    (evil-org-agenda-set-keys)
    ; this essentially resets `org-super-agenda-header-map' to an empty (pass-through) keymap
    (setq org-super-agenda-header-map (make-sparse-keymap)))

  (use-package evil-org
    :hook
    (org-mode . evil-org-mode)
    (org-agenda-mode . set-up-org-agenda-key-bindings))
#+end_src
...
#+begin_src emacs-lisp
  (use-package treemacs
    :defer t
    :general
    (:states 'normal :prefix ", t"
             "t" 'treemacs))
#+end_src

Evil keybindings come in a separate package
