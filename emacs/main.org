...
There's experimental support for inlay hints in typescript language server. Inline hints are actually much nicer in Rust mode, but I haven't been able to configure the face yet. The enum inlay hints are so far causing the language server to error out quite often, so they're disabled.
