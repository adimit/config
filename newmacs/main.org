...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-attribute 'variable-pitch nil :font "FiraGo-14.0" :weight 'light)
...
#+begin_src emacs-lisp
  ...
  (defun insert-knuth-ticket-number-from-branch ()
    ...
    (let ((knuth (get-knuth-number-from-string (magit-get-current-branch))))
      (if (and knuth (not (last-line-matches-p knuth))) (buffer-insert-at-end knuth))))

  (defun last-line-matches-p (needle)
  ...
  Ignores comments"
    (save-excursion
      ...
      (if (= (char-after) ?#)
          nil
        (search-forward needle nil 'noerror))))
